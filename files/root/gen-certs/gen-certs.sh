...
gen::check::req() {
        if [ ! -f "indexer.lst" ] || [ ! -f "server.lst" ] || [ ! -f "dashboard.lst" ]; then
		echo "#########################################################"
		echo "indexer.lst, server.lst or dashboard.lst files not found."
		echo "#########################################################"

		exit;
	fi

	for f in $(echo "indexer.lst server.lst dashboard.lst")
	do
		grep -e "^[^#].*_ip=" ${f}
...
gen::root::ca() {
	...
	$(openssl req -x509 -new -nodes -newkey rsa:2048 -keyout out-certificates/root-ca.key \
		-out out-certificates/root-ca.pem -batch -subj '/OU=Opensearch/O=Opensearch/L=California/' -days 3650)
}
...
gen::admin::cert() {
	...
	$(openssl req -new -key out-certificates/admin-key.pem -out out-certificates/admin.csr \
		-batch -subj '/C=US/L=California/O=Opensearch/OU=Opensearch/CN=admin')
...
gen::init() {
	...
	_indexer=$(cat indexer.lst | sed -e '/^#/d' | sed '/^$/d')
	...
	do
        ...
        _server=$(cat server.lst | sed -e '/^#/d' | sed '/^$/d')
        ...
        _dashboard=$(cat dashboard.lst | sed -e '/^#/d' | sed '/^$/d')
